...
**AI 能力（DeepSeek，全部带防编造约束）**

- 润色 / 一句话生成要点 / 根据简历生成简介 / 整份中英互译（可撤销）/ 求职信初稿
- 简历体检（规则版评分：量化率、弱动词、完整度…零 AI 成本）
- JD 匹配分析（匹配度 / 缺失关键词 / 改进建议）
- **JD 定制 agent**：一键产出岗位定制版简历副本，原件不动
- **简历教练 agent**：访谈式对话，把你的回答实时写进简历（可撤销）
