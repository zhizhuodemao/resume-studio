...
**✦ AI 访谈（产品主线，详见 PRODUCT.md）**

- **访谈舞台**：全屏专注态 —— 一屏一个问题、语音回答（连续模式：自动开麦、说完停顿即自动发送）、素材回执实时滑入、简历缩略图当场长出新内容、结束出「挖掘成果单」
- **素材库**：访谈挖出的故事（STAR + 你亲口说的数字 + 原话出处）沉淀为跨简历复用的资产；旧简历自动反解入库；已问/拒答清单保证教练绝不重复提问（跨会话）
- **三条入口**：贴 JD 定向访谈（一岗一历）/ 贴旧简历 AI 解析入库后深挖 / 直接聊经历；入口可选中英文（联动界面、语音识别与提问语言）
- **一段对话两种镜头**：舞台与侧栏共享同一份对话，关闭/刷新后原地续；侧栏可下达任何编辑指令（agent 真循环：工具结果回喂、自我修正、diff 回执、单轮撤销）
...
- 70 个单元测试（含后端集成）+ 45 个 Playwright 端到端测试（含全栈云同步往返、访谈全链路、跨会话记忆），GitHub Actions CI，顶层错误边界
